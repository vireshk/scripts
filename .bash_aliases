...
alias qemuarmfs="~/work/repos/tools/qemu/qemu/buildarm64/qemu-system-aarch64 -machine virt,virtualization=on -cpu cortex-a57 -machine type=virt -nographic -smp 4 -m 2G -kernel ~/work/repos/tools/qemu/scripts/Image --append \"console=ttyAMA0\" -object memory-backend-file,id=mem,size=2G,mem-path=/dev/shm,share=on -numa node,memdev=mem"
